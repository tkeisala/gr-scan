...
bool scanner_sink::TrySignal(double min, double max)
{
...
void scanner_sink::Rearrange(float *bands, double *freqs, double centre, double bandwidth)
{
...
void scanner_sink::GetBands(float *powers, float *bands, unsigned int bandwidth)
{
...
void scanner_sink::CloseCSV()
{
    fclose(m_outcsv);
}

void scanner_sink::WriteCSV(float freq, float width, float peak, float diff)
{
    time_t timer;
    struct tm *tm_info;
    char       buf[26];
